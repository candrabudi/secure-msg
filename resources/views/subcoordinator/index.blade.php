...
@section('content')
    <div
        ...
        <h2 class="intro-y mt-10 text-lg font-medium">Data List Koordinator</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a href="{{ route('management.subcoordinator.create') }}" data-tw-merge=""
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const subcoordiantorTBody = document.getElementById('product-tbody');
            const perPageSelect = document.getElementById('per-page');
            const productsIndexRoute = @json(route('management.subcoordinator.getSubCoordinator'));
            ...
            function fetchData(page = 1, perPage = 10, search = '') {
                fetch(`${productsIndexRoute}?page=${page}&per_page=${perPage}&search=${encodeURIComponent(search)}`)
                    ...
                    .then(data => {
                        renderSubCoordinators(data.data);
                        renderPagination(data);
                        showDataInfo(data);
                    });
            }

            function renderSubCoordinators(subcoordinators) {
                subcoordiantorTBody.innerHTML = '';
                subcoordinators.forEach(subcoordinator => {
                    subcoordiantorTBody.insertAdjacentHTML('beforeend', `
                    <tr data-tw-merge="" class="intro-x">
                        <td data-tw-merge=""
                            ...
                            ${subcoordinator.username}
                        </td>
                        <td data-tw-merge=""
                            class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                            ${subcoordinator.user_profile.full_name}
                        </td>
                        <td data-tw-merge=""
                            class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                           ${subcoordinator.user_profile.aka_name}
                        </td>
                        <td data-tw-merge=""
                            class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                           ${subcoordinator.user_profile.position}
                        </td>
                        <td data-tw-merge=""
                            class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                           ${subcoordinator.user_profile.national_id}
                        </td>
                        <td data-tw-merge=""
                            ...
                            <div class="flex items-center justify-center">
                                <a class="mr-3 flex items-center" href="/management/subcoordinator/edit/${subcoordinator.id}">
