...
@section('content')
    <div
        ...
        <h2 class="intro-y mt-10 text-lg font-medium">Data List Tipe Tugas</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a href="{{ route('masterdata.tasktype.create') }}" data-tw-merge=""
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const typetaskTBody = document.getElementById('product-tbody');
            const perPageSelect = document.getElementById('per-page');
            const productsIndexRoute = @json(route('masterdata.tasktype.getTaskType'));
            ...
            function fetchData(page = 1, perPage = 10, search = '') {
                fetch(`${productsIndexRoute}?page=${page}&per_page=${perPage}&search=${encodeURIComponent(search)}`)
                    ...
                    .then(data => {
                        renderTypeTasks(data.data);
                        renderPagination(data);
                        showDataInfo(data);
                    });
            }

            function renderTypeTasks(typetasks) {
                typetaskTBody.innerHTML = '';
                typetasks.forEach(typetask => {
                    typetaskTBody.insertAdjacentHTML('beforeend', `
                    <tr data-tw-merge="" class="intro-x">
                        ...
                        <td data-tw-merge=""
                            ...
                            <div class="flex items-center justify-center">
                                <a class="mr-3 flex items-center" href="/master-data/tipe-tugas/edit/${typetask.id}">
